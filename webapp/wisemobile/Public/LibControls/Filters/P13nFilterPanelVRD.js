...
//						text : oCnt_FHelps.f_readTranslate("FILTERPANEL_INCLUDES"),
...
//						text : this._oRb.getText("FILTERPANEL_EXCLUDES"),
	...
	P13nFilterPanel.prototype.Reset= function() {
		...
		this.getParent().getHeaderToolbar().getContent()[3].firePress();
	},
	...
	P13nFilterPanel.prototype.onBeforeRendering = function() {
		...
		if (this._bUpdateRequired) {
			...
			this.getItems().forEach(function(oItem_) {
				...
				// ASVRAMIREZ
				// VALORES ADICIONALES
				if (oItem_.getCustomData().length >= 1){
					for(var i = 0; i < oItem_.getCustomData().length; i++)
						
						switch(oItem_.getCustomData()[i].getKey()){
							case "bindingPath": // PARA COMBOS
								oContext["zbindingPath"] = oItem_.getCustomData()[0].getValue();
								break;
				...
				aKeyFields.push({
					...
					zbindingPath : oContext.getObject()["zbindingPath"]
				});
			...
			this.getFilterItems().forEach(function(oFilterItem_) {
				...
				// ASVRAMIREZ
				// VALORES ADICIONALES
				if (oFilterItem_.getCustomData().length >= 1){
					for(var i = 0; i < oFilterItem_.getCustomData().length; i++)
						
						switch(oFilterItem_.getCustomData()[i].getKey()){
							case "zvisible": // PARA COMBOS
								oContext["zvisible"] = oFilterItem_.getCustomData()[0].getValue();
								break;
				...
				aConditions.push({
					...
					visible : oContext.getObject()["zvisible"]
				});
			});
