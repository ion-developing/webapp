...
sap.ui.define([
		...
		if (this._oConditionsMap) {
			for ( var conditionId in this._oConditionsMap) {
				...
				if (!oCondition._oGrid || oCondition._oGrid.select.getSelected()) {
					...
					var oObj		= {};
					var oKeyField	= aFilter.keyField;
					oObj[oKeyField] = oCondition.value1
					aFilter.zstr	= oCnt_FHelps.f_json2xml(oObj);
					
					// VALIDACION DE FECHA
					if(aFilter.typeField == "date"){
						var oObj 	= {};
						oObj[oKeyField] = { SIGN:"I",OPTION:"BT",
												LOW : aFilter.value1,
												HIGH: (aFilter.value2 == null || aFilter.value2 == "")? aFilter.value1 : aFilter.value2,
										  };
						aFilter.zstr = oCnt_FHelps.f_json2xml(oObj);
					}
	...
	P13nConditionPanel.prototype.init = function() {
		...
		*/
		this._sFromLabelText 	= oCnt_FHelps.f_readTranslate("CONDITIONPANEL_LABELFROM");
		this._sToLabelText 		= oCnt_FHelps.f_readTranslate("CONDITIONPANEL_LABELTO");
		this._sValueLabelText 	= oCnt_FHelps.f_readTranslate("CONDITIONPANEL_LABELVALUE");
		this._sShowIfGroupedLabelText 		= oCnt_FHelps.f_readTranslate("CONDITIONPANEL_LABELGROUPING");
		this._sValidationDialogFieldMessage = oCnt_FHelps.f_readTranslate("CONDITIONPANEL_FIELDMESSAGE");
	...
	P13nConditionPanel.prototype._createValueField = function(oCurrentKeyField, oFieldInfo, oConditionGrid) {
		...
		switch (sCtrlType) {
			...
			case "combo":
				...
				var oBindPath= oCurrentKeyField.zbindingPath.split("-");
				var o_path	= oBindPath[0] == undefined ? "" : oBindPath[0];
				var o_key	= oBindPath[1] == undefined ? "" : oBindPath[1];
				var o_text	= oBindPath[2] == undefined ? "" : oBindPath[2];
				...
				oControl = new sap.m.ComboBox({
					...
					items: {
						...
						template: new sap.ui.core.ListItem({
							key:"{mCombos>"+o_key+"}", text:"{mCombos>"+o_text+"}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			        if (sap.m.ComboBox.prototype.onAfterRendering) {
			        	sap.m.ComboBox.prototype.onAfterRendering.apply(this, arguments);
			        }
			        // Your code...
			...
			case "combo_moneda":
				...
				oControl = new sap.m.ComboBox({
					...
					selectedKey: "",
					items: {
						path: "mCombos>/" + oCurrentKeyField.zbindingPath,
						template: new sap.ui.core.ListItem({
							key:"{mCombos>WAERS}", text:"{mCombos>LTEXT}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			...
			case "combo_estado":
				...
				oControl = new sap.m.ComboBox({
					...
					items: {
						path: "mCombos>/" + oCurrentKeyField.zbindingPath,
						template: new sap.ui.core.ListItem({
							key:"{mCombos>Status}", text:"{mCombos>Statdesc}"
						})
					}
				});
				oControl.onAfterRendering = function() {
			...
			case "date":
				...
				oControl.setDisplayFormat("dd/MM/yyyy");
...
//				sText = this._oRb.getText("CONDITIONPANEL_OPTION" + sType + aOperations[iOperation]);
				sText = oCnt_FHelps.f_readTranslate("CONDITIONPANEL_OPTION" + sType + aOperations[iOperation]);
			}
			else {
