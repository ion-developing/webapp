...
sap.ui.define(
	['jquery.sap.global', 'sap/ui/core/mvc/Controller', 'sap/m/MessageBox'],
	function (jQuery, Controller, MessageBox) {
		var oView
		var oBusyDialog_a

		return Controller.extend(
			'com.axiomasoluciones.wisemobile.Modulo_WiseMobile.Login.controller.Login',
			{
				_formFragments: {},
				...
				onInit: function () {
					...
					imageToolbar.setSrc('wisemobile/Public/Img/logo.png');
					...
					*/
						...
						loginImage.setSrc('wisemobile/Public/Img/logo.png')
						...
						/*
					var imageToolbar = sap.ui.getCore().byId("__image1");
					imageToolbar.setSrc('wisemobile/Public/Img/logo.webp');
					imageToolbar.setHeight("20px");
					...
					*/
						...
						// Login Image
						var loginImage = this.byId('loginViewImage')
						loginImage.setSrc('wisemobile/Public/Img/logo.webp')
						loginImage.setHeight('150px')
					...
					console.log(
						'onInit called - com.axiomasoluciones.wisemobile.Modulo_WiseMobile.Login.controller.Login'
					)
					...
					sap.ui.core.UIComponent.getRouterFor(this).attachRouteMatched(
						function (oEvent) {
							...
							oView_temp.bindElement({
								path: '/Table',
								model: 'mManageControls'
							})
						}
					)
					...
					this._formFragments.Sociedades = sap.ui.xmlfragment(
						'com.axiomasoluciones.wisemobile.Modulo_WiseMobile.GondiQA.view.fragments.Sociedades',
						this
					)
				...
				createContent: function () {
					console.log(
						'createContent called -  com.axiomasoluciones.wisemobile.Modulo_WiseMobile.Login.controller.Login'
					)
				},
				/*
	...
	//				}
				...
				*/
					...
					if (
						oCnt_FHelps.f_Inputs([
							oView.byId('myUser'),
							oView.byId('myPassword')
						]) == true
					) {
						function oFunction(oData) {
							...
							var oObjeto = sap.ui
								...
								.getModel('mUser')
								.getProperty(
									'/SOAP:Envelope/SOAP:Body/n0:_-WI20_-FM_WS_LOGIN_USER.Response/IM_OUT_LOGIN'
								)
							...
							var Type = sap.ui
								...
								.getModel('mUser')
								.getProperty(
									'/SOAP:Envelope/SOAP:Body/n0:_-WI20_-FM_WS_LOGIN_USER.Response/IM_RETURN/item'
								).TYPE
							var Message = sap.ui
								.getCore()
								.getModel('mUser')
							...
							if (Type == 'S') {
								...
								var oDialog = new sap.m.Dialog({
									...
									contentWidth: '30%',
									content: [oCtrl_Login._formFragments.Sociedades],
									buttons: [
										new sap.m.Button({
								...
								oCnt_FHelps.f_showMessage(
									'ERROR',
									Message,
									undefined,
									oCnt_FHelps.f_readTranslate('Error.title')
								)
							}
						}
				...
				oServ_Login: function (callback) {
					...
					function OnError(request, status, error) {
						//Función que se ejecuta si ocurre algún error
						var oText = oCnt_FHelps.f_readTranslate('Login-error')
						oCnt_FHelps.f_showMessage('ERROR', oText)
						oBusyDialog.close()
					}
				...
				oServ_Sociedades_ZW20: function () {
					...
					soapMessage =
						soapMessage + '<IV_USERWEB>' + oUsuario_login + '</IV_USERWEB>'
					soapMessage = soapMessage + '</urn:_-WI20_-FM_WS_GET_COMPANY_LIST>'
					soapMessage = soapMessage + '</soap:Body>'
					soapMessage = soapMessage + '</soap:Envelope>'
					...
					function OnSuccess(data, status) {
						...
						sap.ui
							.getCore()
							.getModel('mCombos')
							.setProperty('/SOCIEDADES_ZW20', json) //revisar datos

						var mData = sap.ui
							.getCore()
							.getModel('mCombos')
							.getProperty(
								'/SOCIEDADES_ZW20/SOAP:Envelope/SOAP:Body/n0:_-WI20_-FM_WS_GET_COMPANY_LIST.Response/ET_BUKRS_LIST/item'
							)
						...
						if (length == undefined) {
							sap.ui
								.getCore()
								.getModel('mCombos')
								.setProperty('/SOCIEDADES_ZW20', [mData]) //revisar datos
						} else {
							sap.ui
								.getCore()
								.getModel('mCombos')
				...
				oServ_Configuracion: function (oObject, callback) {
					...
					soapMessage =
						soapMessage + '<USERWEB>' + oUsuario_login + '</USERWEB>'
					if (oLenguaje == 'ES') {
						soapMessage = soapMessage + '<LANGUAGE>S</LANGUAGE>'
					}
					if (oLenguaje == 'EN') {
				...
				*/
					...
					soapMessage = soapMessage + '<urn:_-WI18_-SUPPLIER_DATA>'
					soapMessage =
						soapMessage + '<IV_BUKRS>' + oObject.BUKRS + '</IV_BUKRS>'
					soapMessage = soapMessage + '<IV_LIFNR>' + Lifnr + '</IV_LIFNR>'
					soapMessage = soapMessage + '</urn:_-WI18_-SUPPLIER_DATA>'
					soapMessage = soapMessage + '</soapenv:Body>'
					...
					function OnSuccess(data, status) {
						...
						var mData = mModel.getProperty(
							'/SOAP:Envelope/SOAP:Body/n0:_-WI18_-SUPPLIER_DATA.Response/ES_SUPP_DATA_OUT'
						)
						Name = mData.NAME1
				...
				onChangeSociedad: function (oEvt) {
					...
					function oFunction(oData) {
						...
						var mData = mModel.getProperty(
							'/SOAP:Envelope/SOAP:Body/n0:_-WI20_-FM_WS_GET_CONFIG.Response/EX_OUT_MENUWEB_V'
						)
						if (mData != undefined) {
							Bname = mData.Bname
							var mDataV = mModel.getProperty(
								'/SOAP:Envelope/SOAP:Body/n0:_-WI20_-FM_WS_GET_CONFIG.Response/EX_OUT_MENUWEB_V'
							)
							...
							mDataV.INV_WO_OC_CFDI =
								mDataV.INV_WO_OC_CFDI == 'X' ? true : false
							mDataV.INV_WO_OC_NORMAL =
								mDataV.INV_WO_OC_NORMAL == 'X' ? true : false
							mDataV.INV_LIST_STATUS =
								mDataV.INV_LIST_STATUS == 'X' ? true : false
							mDataV.INV_LIST_PO = mDataV.INV_LIST_PO == 'X' ? true : false
							mDataV.INV_ACC_STATUS =
								mDataV.INV_ACC_STATUS == 'X' ? true : false
							mDataV.INV_REC_STATUS =
								mDataV.INV_REC_STATUS == 'X' ? true : false
							mDataV.INV_RFQ_CREATE =
								mDataV.INV_RFQ_CREATE == 'X' ? true : false
							mDataV.INV_RFQ_LIST_STATUS =
								mDataV.INV_RFQ_LIST_STATUS == 'X' ? true : false
							mDataV.PAY_LIST_STATUS =
								mDataV.PAY_LIST_STATUS == 'X' ? true : false
							mDataV.INV_MESSAGE = mDataV.INV_MESSAGE == 'X' ? true : false
							mDataV.INV_MAILBOX_W_CFDI =
								mDataV.INV_MAILBOX_W_CFDI == 'X' ? true : false
							mDataV.INV_MAILBOX_WO_CFDI =
								mDataV.INV_MAILBOX_WO_CFDI == 'X' ? true : false
							mDataV.INV_GET_MAILBOX =
								mDataV.INV_GET_MAILBOX == 'X' ? true : false
							mDataV.INV_CHANGE_PASSWORD =
								mDataV.INV_CHANGE_PASSWORD == 'X' ? true : false
							mDataV.INV_BUSINESS_AREA_LIST =
								mDataV.INV_BUSINESS_AREA_LIST == 'X' ? true : false
							mDataV.INV_ADD_OTHER_PDF =
								mDataV.INV_ADD_OTHER_PDF == 'X' ? true : false
							mDataV.INV_WO_OC_CFDI_I =
								mDataV.INV_WO_OC_CFDI_I == 'X' ? true : false
							mDataV.INV_WO_OC_CFDI_I =
								mDataV.INV_WO_OC_CFDI_I == 'X' ? true : false
							mDataV.INV_QUOTATION = mDataV.INV_QUOTATION == 'X' ? true : false
							mDataV.INV_STATUS_QUOTATION =
								mDataV.INV_STATUS_QUOTATION == 'X' ? true : false
							mDataV.INV_CREDIT_MEMO =
								mDataV.INV_CREDIT_MEMO == 'X' ? true : false
							mDataV.INV_WO_CREDIT_MEMO =
								mDataV.INV_WO_CREDIT_MEMO == 'X' ? true : false
							mDataV.INV_CONSIGM = mDataV.INV_CONSIGM == 'X' ? true : false
							mDataV.PAC_VALIDATION =
								mDataV.PAC_VALIDATION == 'X' ? true : false
				...
				onChangeIdioma: function (language) {
					...
					oCnt_FHelps.languageChange(oId, function () {
							...
							new sap.m.VBox({
								items: [
									new sap.ui.layout.form.SimpleForm({
										...
										content: [
											new sap.m.Label({
												text: '{i18n>OlvideContrasena.Label.1}'
											}),
											aInputs[0]
									...
									if (oCnt_FHelps.f_Inputs([aInputs[0]]) == true) {
										...
										function oFunction(oData) {
											...
											var oObjeto = sap.ui
												...
												.getModel('mMessages')
												.getProperty(
													'/SOAP:Envelope/SOAP:Body/n0:_-WI20_-FM_WS_SEND_TOKEN.Response/EX_RETURN'
												)
											...
											if (oObjeto.TYPE == 'S') {
															...
															items: [
																new sap.ui.layout.form.SimpleForm({
																	...
																	content: [
																		...
																		aInputs1[0],
																		new sap.m.Label({
																			text: '{i18n>OlvideContrasena.Label.2}'
																		}),
																		aInputs1[1],
																		new sap.m.Label({
																			text: '{i18n>OlvideContrasena.Label.3}'
																		}),
																		aInputs1[2],
																		new sap.m.Label({
																			text: '{i18n>OlvideContrasena.Label.4}'
																		}),
																		aInputs1[3],
																		new sap.m.Label({
																			text: '{i18n>OlvideContrasena.Label.5}'
																		}),
																		aInputs1[4]
																	]
																})
																	...
																	if (
																		...
																		function oFunction(oData) {
																			...
																			var oObjeto = sap.ui
																				...
																				.getModel('mMessages')
																				.getProperty(
																					'/SOAP:Envelope/SOAP:Body/n0:_-WI20_-FM_WS_VALIDATE_TOKEN.Response/EX_RETURN'
																				)
																			console.log(oObjeto)
																			...
																			if (oObjeto.TYPE == 'S') {
																				function oFunction1(oData) {
																					...
																					var oObjeto = sap.ui
																						...
																						.getModel('mCambio')
																						.getProperty(
																							'/SOAP:Envelope/SOAP:Body/n0:_-WI20_-FM_WS_PASS_CHANGE.Response/EX_OUT_RETURN/item'
																						)
																					console.log(oObjeto)
																				...
																				oC_Modulo_WiseMobile.oServ_CambioPass(
																					oObject,
																					oFunction1
																				)
																			} else {
																		...
																		oCtrl_Login.oServ_ValidateToken(
																			oObject,
																			oFunction
																		)
																	} else {
												...
												oCnt_FHelps.f_showMessage(
													...
													oObjeto.MESSAGE,
													undefined,
													oCnt_FHelps.f_readTranslate('Mensaje.title')
												)
											} else {
			...
			*/
				...
				oServ_EnvioToken: function (oObject, callback) {
					...
					webServiceURL_ZW20 =
						ip_apache_dev + oPath_login_ini + oMandante + oPath_login_fin
					console.log(oPath_login_fin)
					console.log(webServiceURL_ZW20)

					oBusyDialog_a.setText(
						oCnt_FHelps.f_readTranslate('OlvideContrasena.Message.1')
					)
					oBusyDialog.open()
					//Llamamos a la función AJAX de JQuery
					$.ajax({
